...
- SPEC-149 fixed misleading auth/security/RBAC documentation: corrected `clientWssCode` in security.mdx (removed non-existent `token` field, added `storage`, used `setAuthToken()`), corrected false "default-deny" claims in rbac.mdx to accurately describe default-allow model. 2 commits.
