...
Total: approximately **60+ distinct Rust struct/enum definitions** across all schema domains.
...
- `HybridQueryRespPayload`, `HybridQueryDeltaPayload`
- `LockGrantedPayload`, `LockReleasedPayload`
...
- **Client-event payload types** (HybridQueryRespPayload, LockGrantedPayload, etc.) that are not in the MessageSchema union are still needed as standalone Rust structs for server-to-client message construction.
...
---
*Generated by SpecFlow on 2026-02-14. This spec requires `/sf:split` before implementation.*
