...
export function Dashboard() {
  ...
  const totalOps = enrichedMembers.reduce(
    (acc, member) => acc + getMetricValue(member.stats.topgun_ops_total),
    0,
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        ...
        <div className="bg-card p-6 rounded-lg shadow border border-border">
          ...
          <p className="text-4xl font-bold text-foreground">
            {formatBytes(
              enrichedMembers.reduce(
                (acc, member) => acc + getMetricValue(member.stats.topgun_memory_usage_bytes),
                0,
              ),
            )}
          </p>
        ...
        <div className="bg-card p-6 rounded-lg shadow border border-border">
          ...
          <p className="text-4xl font-bold text-foreground">
            {enrichedMembers.reduce(
              (acc, member) => acc + getMetricValue(member.stats.topgun_connected_clients),
              0,
            )}
          </p>
        </div>
