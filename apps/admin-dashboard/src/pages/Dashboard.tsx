        ...
        <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                <div className="bg-white p-6 rounded shadow">
                    <h3 className="text-gray-500 text-sm font-bold uppercase">Cluster Size</h3>
                    <p className="text-4xl font-bold">{members.length}</p>
                </div>
                <div className="bg-white p-6 rounded shadow">
                    <h3 className="text-gray-500 text-sm font-bold uppercase">Total Ops</h3>
                    <p className="text-4xl font-bold">
                        {enrichedMembers.reduce((acc, member) => acc + getMetricValue(member.stats.topgun_ops_total), 0)}
                    </p>
                </div>
                <div className="bg-white p-6 rounded shadow">
                    <h3 className="text-gray-500 text-sm font-bold uppercase">Memory Usage</h3>
                    <p className="text-4xl font-bold">
                        {formatBytes(enrichedMembers.reduce((acc, member) => acc + getMetricValue(member.stats.topgun_memory_usage_bytes), 0))}
                    </p>
                </div>
                <div className="bg-white p-6 rounded shadow">
                    <h3 className="text-gray-500 text-sm font-bold uppercase">Connected Clients</h3>
                    <p className="text-4xl font-bold">
                                    ...
                                    <div className="flex items-center">
                                        <div className="ml-3">
                                            <p className="text-gray-900 whitespace-no-wrap">
                                                {member.id}
                                                {member.isLocal && <span className="ml-2 text-xs bg-green-200 text-green-800 px-2 py-1 rounded">YOU</span>}
                                            </p>
                                        </div>
                                    </div>
                                </td>
