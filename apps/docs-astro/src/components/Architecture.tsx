...
const Connection = ({
  ...
  <div className="h-[2px] w-12 md:w-24 bg-black/10 dark:bg-white/10 relative overflow-hidden">
    {active && (
      <div
        className={`absolute inset-0 bg-gradient-to-r from-transparent via-brand-subtle to-transparent w-1/2 ${reverse ? 'animate-[sync-pulse_2s_ease-in-out_infinite_reverse]' : 'animate-[sync-pulse_2s_ease-in-out_infinite]'}`}
      />
    )}
  </div>
);
...
export const Architecture = () => {
  return (
    <section
      id="architecture"
      className="py-24 border-t border-card-border bg-neutral-50 dark:bg-background transition-colors"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-3xl font-bold text-foreground mb-4">Architecture</h2>
          <p className="text-neutral-600 dark:text-neutral-300 max-w-2xl mx-auto">
            TopGun inverts the traditional model. The client is the primary source of truth for the
            UI, while the server ensures eventual consistency and persistence.
          </p>
        </div>
        ...
        <div className="relative p-8 md:p-12 rounded-3xl border border-card-border bg-grid overflow-hidden bg-white dark:bg-card">
          ...
          <div className="relative flex flex-col md:flex-row items-center justify-center gap-4 md:gap-0">
            ...
            <div className="flex flex-col gap-6 p-6 rounded-2xl border border-dashed border-neutral-300 dark:border-white/10 bg-neutral-100/50 dark:bg-white/[0.02]">
              <div className="text-xs font-mono text-neutral-500 uppercase tracking-widest text-center mb-2">
                Client Device
              </div>
              ...
              <div className="flex justify-center">
                <div className="px-4 py-2 rounded-full bg-brand-subtle/10 border border-brand-subtle/20 text-brand text-xs font-mono">
                  Local CRDT Store
                </div>
              </div>
            </div>
