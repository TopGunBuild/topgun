    ...
    >
      ...
      {isOnline && (
        <div className="absolute inset-0 origin-bottom-left animate-[spin_4s_linear_infinite] opacity-10 pointer-events-none">
          <div
            className="h-full w-full bg-gradient-to-tr from-transparent via-transparent to-green-500/50"
      ...
      {draggingId && dragPosition && (
        <>
          <div
            className="absolute top-0 bottom-0 border-l border-dashed border-brand-subtle/30 pointer-events-none z-0"
            style={{ left: `${dragPosition.x}%` }}
          ></div>
          <div
            className="absolute left-0 right-0 border-t border-dashed border-brand-subtle/30 pointer-events-none z-0"
            style={{ top: `${dragPosition.y}%` }}
          ></div>
          <div
            className="absolute font-mono text-xs text-brand-muted bg-slate-900/80 px-1.5 py-0.5 rounded -translate-y-6 pointer-events-none z-30"
        ...
        return (
          ...
          >
            ...
            <div className="relative group">
              <div
                className={`relative flex items-center justify-center w-6 h-6 rounded-full border-2 bg-slate-900 transition-colors ${
                  isSelected || isDragging
                    ? 'border-white scale-125 z-20 shadow-[0_0_15px_rgba(255,255,255,0.5)]'
                    : drone.status === DroneStatus.COMBAT
                      ? 'border-red-500'
                      : 'border-brand-subtle'
                }`}
              >
                <div
                  className={`w-2 h-2 rounded-full ${drone.status === DroneStatus.COMBAT ? 'bg-red-500' : 'bg-brand-subtle'}`}
                ></div>
              </div>

              {/* Label - always visible when selected/dragging, hover otherwise */}
              <div
                className={`absolute top-7 left-1/2 -translate-x-1/2 whitespace-nowrap z-30 pointer-events-none transition-opacity duration-150 ${
