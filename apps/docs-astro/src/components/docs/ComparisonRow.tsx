...
const CellContent = ({ value, isTopGun: _isTopGun = false }: { value: CellValue; isTopGun?: boolean }) => {
  ...
  return (
    <span
      className={`flex items-center gap-1 justify-center ${value.variant === 'error' ? 'text-red-500 dark:text-red-400' : ''}`}
    >
...
export const ComparisonRow = ({ feature, tg, es, fb, rx }: ComparisonRowProps) => (
  <tr className="border-b border-card-border hover:bg-black/5 dark:hover:bg-white/[0.02] transition-colors">
    ...
    <td className="py-4 px-4 text-center bg-brand-subtle/5 border-x border-brand-subtle/10">
      <div className="flex justify-center text-brand font-semibold text-sm">
