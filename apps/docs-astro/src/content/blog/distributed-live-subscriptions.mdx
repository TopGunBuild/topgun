...
> **Correction (June 2026): this describes a design that never shipped.** TopGun is **single-node stable** today. The distributed cross-node subscription system below — the coordinator, the four-message `CLUSTER_SUB_*` protocol, cross-node RRF merging, and self-healing on node failure — is **not implemented** in the released server. The cluster message types exist only as wire payloads; the server explicitly rejects them as future (Phase 3+) work, and no coordinator exists. Live subscriptions and full-text search work on a single node. Cross-node distributed subscriptions remain a future roadmap item and are **not available**. This post is kept for historical context; do not treat it as a description of shipped behavior.
...
To be clear about what shipped: `searchSubscribe()` and `query().subscribe()` work today on a **single node**. The cross-node distributed coordinator described above was a design that has not been built — there are no code changes that make these subscriptions span a cluster, because the cluster-side machinery does not exist in the released server.

For what is actually shipped today, see:
- [Search & live queries](/docs/guides/search-and-live-queries) — single-node live query and search subscriptions
- [Roadmap](/docs/roadmap) — current cluster maturity and what is still planned

Real-time works on a single node today. Cross-node distributed subscriptions remain a roadmap item.
