...
- **Pluggable Persistence:** TopGun ships with an embedded **redb** backend by default (zero-config local development) and supports **PostgreSQL** for production. TopGun acts as the high-speed in-memory cache and synchronization layer in front of whichever backend you choose.
