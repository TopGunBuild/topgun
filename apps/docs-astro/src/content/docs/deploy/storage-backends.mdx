...
Choose how the TopGun server persists data. The backend is selected at startup via `STORAGE_BACKEND` (source: `packages/server-rust/src/bin/test_server.rs:109`). Three choices are available: `redb` (embedded, default), `postgres` (PostgreSQL, production), and `null` (ephemeral, tests only).
...
Source: `STORAGE_BACKEND` at `bin/test_server.rs:109`; `TOPGUN_REDB_PATH` at `bin/test_server.rs:115`.
...
Source: `STORAGE_BACKEND` at `bin/test_server.rs:109`; `DATABASE_URL` at `bin/test_server.rs:126`.
...
The `null` backend is used in the TopGun integration test suite for deterministic, stateless test runs. Source: `bin/test_server.rs:109` — `"null"` is one of the three accepted values.
