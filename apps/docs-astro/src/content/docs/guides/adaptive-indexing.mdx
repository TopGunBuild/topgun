...
<AlertBox variant="danger" title="Not Implemented — Design Only" text="Adaptive Indexing does not exist in the current Rust server. The API described below is a design sketch for a future release, not reference documentation (see /docs/roadmap)." />
...
**Adaptive Indexing** is a planned design for future releases. The Rust server does not implement it today. The API described below is how we intend to ship it — see [/docs/roadmap](/docs/roadmap) for status.
