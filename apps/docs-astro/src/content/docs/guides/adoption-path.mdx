...
export const tier1ServerCode = `# Start the TopGun server (zero-config embedded storage)
pnpm start:server`;
...
function KanbanBoard({ boardId }: { boardId: string }) {
  ...
  const { update } = useMutation('kanban-' + boardId);

  const moveCard = (cardId: string, column: string) => {
    update(cardId, { column, movedAt: Date.now() });
  };

  return (
    <div>{cards.map(card => (
      <div key={card._key}>{card.column}: {card._key}</div>
    ))}</div>
  );
}`;
...
<div className="bg-brand-subtle/10 border border-brand-subtle/20 rounded-xl p-6 my-6 not-prose">
  ...
  <span className="text-brand">For the full Rust server configuration including the embed API (<code className="px-1.5 py-0.5 rounded bg-brand-subtle/10 text-sm">NetworkConfig</code>, <code className="px-1.5 py-0.5 rounded bg-brand-subtle/10 text-sm">TlsConfig</code>), see the <a href="/docs/reference/server" className="underline">Server API reference</a>.</span>
</div>
