...
Without this block, agents reliably hallucinate methods like `client.subscribe()` or reach for `client.getMap()` for reads (which won't re-render on data changes).
