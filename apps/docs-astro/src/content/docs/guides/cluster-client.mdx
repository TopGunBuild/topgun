...
export const quickStartCode = `import { TopGunClient } from '@topgunbuild/client';
...
// Wrap your app with TopGunProvider
// <TopGunProvider client={client}><TodoApp /></TopGunProvider>`;
...
// Hooks continue working through failover — routing is transparent

function TodoApp() {
  const { data: todos } = useQuery<Todo>('todos');
  const { create } = useMutation<Todo>('todos');
  // This works even if the primary node is down —
  // hook writes queue locally and sync when a healthy node responds
  const add = () => create(crypto.randomUUID(), { text: 'New item', done: false });
...
<div className="flex items-center gap-3 mb-6 not-prose">
  <Network className="w-10 h-10 text-brand" />
...
<div className="bg-yellow-50 dark:bg-yellow-900/20 border-l-4 border-yellow-500 p-4 my-6 not-prose">
  <p className="text-sm text-yellow-800 dark:text-yellow-200">
    ...
    the client will try the next one. You don't need to list all nodes—the client discovers them automatically.
  </p>
</div>
...
<div className="bg-brand-subtle/10 border border-brand-subtle/20 rounded-xl p-6 my-6 not-prose">
  <h4 className="font-semibold text-brand mb-2">Offline Support</h4>
