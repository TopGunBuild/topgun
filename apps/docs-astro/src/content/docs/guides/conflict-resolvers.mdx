...
export const clientApiCode = `const resolvers = client.getConflictResolvers();
...
export const reactHooksApiCode = `// Manage resolvers for a map
...
<AlertBox variant="danger" title="Not Implemented — Design Only" text="Custom Conflict Resolvers do not exist in the current Rust server. The server uses LWW resolution only. The API described below is a design sketch for a future release, not reference documentation (see /docs/roadmap)." />
...
**Conflict Resolvers** are a planned design for future releases. The Rust server does not implement them today — it uses LWW (Last-Write-Wins) resolution only. The API described below is how we intend to ship it — see [/docs/roadmap](/docs/roadmap) for status. LWW fails for business-critical cases such as:
...
<div className="not-prose mb-8 rounded-lg border border-brand-subtle/20 bg-brand-subtle/10 p-4">
  <div className="flex items-start gap-3">
    <Shield className="h-5 w-5 text-brand-subtle mt-0.5" />
    <div>
      <p className="font-medium text-brand-subtle">Deletion Handling</p>
      <p className="text-sm text-slate-300 mt-1">
...
<CodeBlock code={clientApiCode} lang="typescript" title="Client API" />

### React Hooks

<CodeBlock code={reactHooksApiCode} lang="typescript" title="React Hooks" />
...
<div className="flex justify-between pt-10 mt-10 border-t border-card-border not-prose">
  ...
  <a href="/docs/guides/interceptors" className="group flex flex-col items-end gap-1 px-6 py-4 rounded-lg border border-card-border hover:border-neutral-400 dark:hover:border-neutral-700 transition-colors">
    <span className="text-xs text-neutral-500">Next</span>
    <div className="flex items-center gap-2 text-foreground font-semibold">
      Interceptors <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
    </div>
  </a>
</div>
