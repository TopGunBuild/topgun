...
<AlertBox variant="warning" title="Planned Feature" text="Custom conflict resolvers are on the roadmap but not yet implemented in the current Rust server. The server currently uses LWW (Last-Write-Wins) resolution only." />
