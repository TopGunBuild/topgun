...
|---|---|---|
| Single-node | Stable | Fencing tokens, TTL-based expiry, disconnect-release API in place |
...
| Cluster (planned) | Raft-replicated | Via openraft — tracked internally as the Raft track |
...
See the [Roadmap](/docs/roadmap) for more details on when and what the Raft track delivers.
...
<div className="flex justify-between pt-10 mt-10 border-t border-card-border not-prose">
  <a href="/docs/guides/interceptors" className="group flex flex-col gap-1 px-6 py-4 rounded-lg border border-card-border hover:border-neutral-400 dark:hover:border-neutral-700 transition-colors">
    <span className="text-xs text-neutral-500">Previous</span>
    <div className="flex items-center gap-2 text-foreground font-semibold">
      <ChevronRight className="w-4 h-4 rotate-180 group-hover:-translate-x-1 transition-transform" /> Interceptors
    </div>
  </a>
