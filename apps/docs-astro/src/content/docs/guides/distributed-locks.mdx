...
<AlertBox variant="warning" title="Planned Feature" text="Distributed locks are on the roadmap but not yet implemented in the current Rust server. The coordination service exists as a stub." />
