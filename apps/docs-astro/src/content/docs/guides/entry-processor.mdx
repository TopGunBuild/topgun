...
<AlertBox variant="warning" title="Planned Feature" text="Entry processors are on the roadmap but not yet implemented in the current Rust server. The persistence service exists as a stub." />
