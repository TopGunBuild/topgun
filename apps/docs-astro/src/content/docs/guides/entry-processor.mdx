...
<AlertBox variant="danger" title="Not Implemented — Design Only" text="Entry Processor does not exist in the current Rust server. The API described below is a design sketch for a future release, not reference documentation (see /docs/roadmap)." />
...
**Entry Processor** is a planned design for future releases. The Rust server does not implement it today. The API described below is how we intend to ship it — see [/docs/roadmap](/docs/roadmap) for status.
...
<div className="grid md:grid-cols-3 gap-4 my-6 not-prose">
  ...
  <div className="p-4 bg-brand-subtle/10 rounded-lg border border-brand-subtle/20">
    <Server className="w-6 h-6 text-brand-subtle mb-2" />
    <h3 className="font-semibold text-brand">Server-Side Execution</h3>
    <p className="text-sm text-brand">Logic runs in secure sandbox on the server</p>
  </div>
...
<div className="bg-brand-subtle/10 border-l-4 border-brand-subtle p-4 my-6 not-prose">
  <p className="text-sm text-brand">
    <strong>Sandboxed Execution:</strong> Processor code runs in an isolated sandbox using <code className="bg-brand-subtle/10 px-1 rounded">isolated-vm</code>.
    It has no access to Node.js APIs, filesystem, network, or globals. Only <code className="bg-brand-subtle/10 px-1 rounded">value</code>, <code className="bg-brand-subtle/10 px-1 rounded">key</code>, and <code className="bg-brand-subtle/10 px-1 rounded">args</code> are available.
  </p>
</div>
