...
export const serverConfigCode = `# Start the TopGun server with Event Journal enabled
# The journal captures all map changes as an append-only log
PORT=8080 \\
DATABASE_URL=postgres://user:pass@localhost/myapp \\
topgun-server

# The Rust server includes built-in event journal functionality
# that persists change events to PostgreSQL for CDC, audit trails,
# and event replay.
...
<div className="grid md:grid-cols-3 gap-4 my-6 not-prose">
  ...
  <div className="p-4 bg-brand-subtle/10 rounded-lg border border-brand-subtle/20">
    <Activity className="w-6 h-6 text-brand-subtle mb-2" />
    <h3 className="font-semibold text-brand">Real-time Streaming</h3>
    <p className="text-sm text-brand">Subscribe to live changes via WebSocket</p>
  </div>
...
<div className="not-prose">
  <CodeBlock title="Server Setup" language="bash" code={serverConfigCode} />
</div>
...
<div className="bg-brand-subtle/10 border-l-4 border-brand-subtle p-4 my-6 not-prose">
  <p className="text-sm text-brand">
    <strong>Real-time vs Async:</strong> For synchronous ML processing (blocking writes until ML completes), use <a href="/docs/guides/interceptors" className="text-brand underline hover:no-underline">Interceptors</a> instead. Event Journal is better for async pipelines where you don't need to block the write operation.
  </p>
</div>
...
<div className="bg-brand-subtle/10 border-l-4 border-brand-subtle p-4 my-6 not-prose">
  <p className="text-sm text-brand">
    <strong>Performance Tip:</strong> For high-volume deployments, consider time-based partitioning of the <code className="bg-brand-subtle/10 dark:bg-brand-muted px-1 rounded">event_journal</code> table and schedule regular cleanup using <code className="bg-brand-subtle/10 dark:bg-brand-muted px-1 rounded">cleanupOldEvents()</code>.
  </p>
</div>
