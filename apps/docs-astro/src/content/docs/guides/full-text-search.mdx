...
export const serverSearchSetupCode = `# The Rust server includes built-in Tantivy-powered full-text search
# Search indexes are automatically maintained for configured maps
PORT=8080 \\
DATABASE_URL=postgres://user:pass@localhost/myapp \\
topgun-server

# Full-text search is enabled by default for all maps
# The server automatically indexes data and handles search queries
...
export const hybridQueryBasicCode = `import { useHybridQuery } from '@topgunbuild/react';
...
export const hybridQueryDynamicCode = `import { useHybridQuery } from '@topgunbuild/react';
import { Predicates } from '@topgunbuild/core';
...
export const hybridQueryClientCode = `import { TopGunClient } from '@topgunbuild/client';
import { Predicates } from '@topgunbuild/core';
...
export const serverSearchPermissionsCode = `# Search respects the RBAC security model.
# Users need READ permission on a map to search it.
#
# Example RBAC policy structure:
#   articles:
#     read:  [user, admin]   # Can search and read
#     write: [admin]         # Can add/update documents
#
# Start the server with JWT auth for role verification:
PORT=8080 \\
DATABASE_URL=postgres://user:pass@localhost/myapp \\
JWT_SECRET=your-secret-key \\
topgun-server

# For the full RBAC and search configuration API,
# see: /docs/reference/server and /docs/guides/rbac`;
...
<div className="not-prose">
  <CodeBlock title="Server Setup" language="bash" code={serverSearchSetupCode} />
</div>
...
<div className="not-prose">
  <CodeBlock title="Permissions" language="bash" code={serverSearchPermissionsCode} />
</div>
...
<div className="bg-purple-50 dark:bg-purple-900/20 border-l-4 border-purple-500 p-4 my-6 not-prose">
  <p className="text-sm text-purple-800 dark:text-purple-200">
    <strong>Scalability:</strong> Each node evaluates subscriptions locally against its FTS index. Update messages flow only to coordinators with active subscriptions, making this approach efficient even with many concurrent subscriptions.
  </p>
</div>
...
<div className="grid md:grid-cols-2 gap-6 mb-8 not-prose">
  <FeatureCard
    ...
    iconColor="text-green-500"
    title="FTS + Filters"
    description="Combine match() with equal(), greaterThan(), between() in one query."
  />
  <FeatureCard
    icon={Star}
    iconColor="text-purple-500"
    title="_score Sorting"
    description="Sort results by BM25 relevance score alongside other sort fields."
  />
</div>
...
<div className="not-prose">
  <CodeBlock title="Basic Hybrid Query" language="typescript" code={hybridQueryBasicCode} />
</div>
...
<div className="not-prose">
  <CodeBlock title="Dynamic Filters" language="typescript" code={hybridQueryDynamicCode} />
</div>
...
<div className="not-prose">
  <CodeBlock title="Client API" language="typescript" code={hybridQueryClientCode} />
</div>
...
<div className="bg-purple-50 dark:bg-purple-900/20 border-l-4 border-purple-500 p-4 my-6 not-prose">
  <p className="text-sm text-purple-800 dark:text-purple-200">
    <strong>When to use Hybrid vs useSearch:</strong> Use <code>useSearch</code> for pure text search (search box). Use <code>useHybridQuery</code> when you need to combine text search with filters (faceted search, filtered listings).
  </p>
</div>
