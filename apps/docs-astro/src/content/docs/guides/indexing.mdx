...
<AlertBox variant="danger" title="Not Implemented — Design Only" text="Indexing does not exist in the current Rust server. The API described below is a design sketch for a future release, not reference documentation (see /docs/roadmap)." />
...
**Indexing** is a planned design for future releases. The Rust server does not implement it today. The API described below is how we intend to ship it — see [/docs/roadmap](/docs/roadmap) for status.
...
<div className="bg-brand-subtle/10 border-l-4 border-brand-subtle p-4 my-6 not-prose">
  <div className="flex items-center gap-2 mb-2">
    <Lightbulb className="w-5 h-5 text-brand-subtle" />
    <strong className="text-brand">When to Add Indexes</strong>
  </div>
...
<div className="flex justify-between pt-10 mt-10 border-t border-card-border not-prose">
  <a href="/docs/guides/schema" className="group flex flex-col gap-1 px-6 py-4 rounded-lg border border-card-border hover:border-neutral-400 dark:hover:border-neutral-700 transition-colors">
    <span className="text-xs text-neutral-500">Previous</span>
    <div className="flex items-center gap-2 text-foreground font-semibold">
      <ChevronRight className="w-4 h-4 rotate-180 group-hover:-translate-x-1 transition-transform" /> Schema &amp; Type Safety
    </div>
  </a>
