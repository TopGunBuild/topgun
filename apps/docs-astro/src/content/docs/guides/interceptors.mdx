...
export const basicInterceptorCode = `# The Rust server provides built-in interceptor functionality
# via its Tower middleware pipeline. Configure via environment variables:
PORT=8080 \\
DATABASE_URL=postgres://user:pass@localhost/myapp \\
topgun-server

# The Rust server includes built-in middleware for:
# - Rate limiting
# - Load shedding
# - Request timeouts
# - Metrics collection
#
# For the full Rust server API and middleware configuration,
...
export const rateLimitCode = `# The Rust server includes built-in rate limiting via Tower middleware
# Rate limiting is enabled by default in production mode
PORT=8080 \\
DATABASE_URL=postgres://user:pass@localhost/myapp \\
topgun-server

# Rate limiting is configured via the LoadShed and Timeout
# middleware layers in the operation pipeline.
# See the Server API reference for details: /docs/reference/server`;

export const chainingCode = `# The Rust server uses a Tower middleware pipeline
# Middleware layers are applied in order:
#   1. LoadShed    - reject requests when overloaded
#   2. Timeout     - enforce request deadlines
#   3. Metrics     - collect prometheus metrics
#   4. Router      - dispatch to domain services
#
# For custom server-side logic, use the Rust embed API
# to build a custom Tower service pipeline.
# See the Server API reference: /docs/reference/server`;
...
<AlertBox variant="danger" title="Not Implemented — Design Only" text="Interceptors do not exist in the current Rust server. The API described below is a design sketch for a future release, not reference documentation (see /docs/roadmap)." />
...
**Interceptors** are a planned design for future releases. The Rust server does not implement them today. The API described below is how we intend to ship it — see [/docs/roadmap](/docs/roadmap) for status.
...
<div className="grid md:grid-cols-3 gap-4 my-6 not-prose">
  ...
  <div className="p-4 bg-brand-subtle/10 rounded-lg border border-brand-subtle/20">
    <Filter className="w-6 h-6 text-brand-subtle mb-2" />
    <h3 className="font-semibold text-brand">Validation & Enrichment</h3>
    <p className="text-sm text-brand">Validate data, add server timestamps, enrich with external data</p>
  </div>
...
<div className="not-prose">
  <CodeBlock title="terminal" language="bash" code={basicInterceptorCode} />
</div>
...
<div className="bg-brand-subtle/10 border-l-4 border-brand-subtle p-4 my-6 not-prose">
  <p className="text-sm text-brand">
    <strong>Performance Tip:</strong> For high-throughput scenarios, consider caching ML responses, using connection pooling for external APIs, or offloading non-critical enrichment to <code className="bg-brand-subtle/10 px-1 rounded">onAfterOp</code> to avoid blocking writes.
  </p>
</div>
...
<div className="not-prose">
  <CodeBlock title="Rate Limiting" language="bash" code={rateLimitCode} />
</div>
...
<div className="not-prose">
  <CodeBlock title="Middleware Pipeline" language="bash" code={chainingCode} />
</div>
