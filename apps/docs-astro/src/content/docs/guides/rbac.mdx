...
export const configCode = `# The Rust server configures RBAC via security policies
# Policies define role-based permissions for map access patterns
#
...
#   ADMIN -> *                 -> ALL
#
# Start the server with JWT authentication for role verification:
PORT=8765 \\
DATABASE_URL=postgres://user:pass@localhost/myapp \\
JWT_SECRET=your-secret-key \\
topgun-server

# For the full RBAC configuration API, see: /docs/reference/server`;
...
RBAC policies are configured on the Rust server via security settings and environment variables.

<div className="not-prose">
  <CodeBlock title="RBAC Configuration" language="bash" code={configCode} />
</div>
