...
export const serverTlsCode = `# Start the TopGun server with TLS enabled
PORT=443 \\
...
<div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-100 dark:border-yellow-800 rounded-xl p-4 mb-6 not-prose">
  <span className="text-sm text-yellow-700 dark:text-yellow-400">The environment variables below are planned for the future production binary. Currently, TLS is configured programmatically via the <code>TlsConfig</code> struct when embedding the server in a Rust application. See the <a href="/docs/reference/server" className="underline">Server API reference</a> for the programmatic API.</span>
</div>
