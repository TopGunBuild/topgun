...
<AlertBox variant="danger" title="Production Deployment — Read This First" text="TopGun's security model is partially shipped. JWT authentication, map-level ACLs, HLC sanitization, trusted-origin pipeline, and client-side EncryptedStorageAdapter are stable and enforced today. The standalone topgun-server CLI binary with env-var TLS configuration, mTLS for cluster communication, and a first-class RBAC role abstraction are not yet implemented. For production today: embed topgun-server programmatically with TlsConfig, or terminate TLS at a reverse proxy. See /docs/roadmap for maturity status." />
...
<div className="flex items-center gap-3 mb-6 not-prose">
  ...
  <p className="text-lg text-neutral-600 dark:text-neutral-300">
    TopGun ships JWT authentication, map-level ACL enforcement, HLC sanitization, trusted-origin pipeline, and client-side <code>EncryptedStorageAdapter</code> today. The standalone CLI binary with env-var TLS config, mTLS for cluster communication, and a first-class RBAC role abstraction are not yet available — see the Production Deployment banner above and the Deployment Mode Semantics section below, or visit <a href="/docs/roadmap">/docs/roadmap</a> for maturity status.
  </p>
</div>
  ...
  <table className="w-full text-left border-collapse text-sm">
    <thead>
      <tr className="border-b border-card-border">
        <th className="py-3 px-4 font-medium text-neutral-500">Capability</th>
        <th className="py-3 px-4 font-medium text-neutral-500">Status</th>
      </tr>
    </thead>
    <tbody>
      <tr className="border-b border-card-border">
        <td className="py-3 px-4 font-medium text-foreground">JWT Authentication</td>
        <td className="py-3 px-4 text-green-600 dark:text-green-400">Stable — HMAC + RSA, <code>JWT_SECRET</code> env var works today</td>
      </tr>
      <tr className="border-b border-card-border">
        <td className="py-3 px-4 font-medium text-foreground">Map-level ACL</td>
        <td className="py-3 px-4 text-green-600 dark:text-green-400">Stable — per-connection, per-map read/write permissions via <code>WriteValidator</code></td>
      </tr>
      <tr className="border-b border-card-border">
        <td className="py-3 px-4 font-medium text-foreground">HLC Sanitization</td>
        <td className="py-3 px-4 text-green-600 dark:text-green-400">Stable — server-generated timestamps replace client-provided ones</td>
      </tr>
      <tr className="border-b border-card-border">
        <td className="py-3 px-4 font-medium text-foreground">Value Size Limits</td>
        <td className="py-3 px-4 text-green-600 dark:text-green-400">Stable — enforced via <code>SecurityConfig</code></td>
      </tr>
      <tr className="border-b border-card-border">
        <td className="py-3 px-4 font-medium text-foreground">Trusted-Origin Bypass</td>
        <td className="py-3 px-4 text-green-600 dark:text-green-400">Stable — <code>CallerOrigin::Forwarded</code>/<code>Backup</code>/<code>Wan</code>/<code>System</code> skip <code>WriteValidator</code></td>
      </tr>
      <tr className="border-b border-card-border">
        <td className="py-3 px-4 font-medium text-foreground">TLS for Client Connections</td>
        <td className="py-3 px-4 text-yellow-600 dark:text-yellow-400">Partial — programmatic via <code>TlsConfig</code> stable; standalone binary with env-var config planned</td>
      </tr>
      <tr className="border-b border-card-border">
        <td className="py-3 px-4 font-medium text-foreground">mTLS for Cluster Communication</td>
        <td className="py-3 px-4 text-red-600 dark:text-red-400">Not implemented — planned v3.0; cluster traffic is plaintext TCP today</td>
      </tr>
      <tr className="border-b border-card-border">
        <td className="py-3 px-4 font-medium text-foreground">First-Class RBAC Roles</td>
        <td className="py-3 px-4 text-red-600 dark:text-red-400">Not implemented as a dedicated surface — ACL + auth-backed principals cover most use cases</td>
      </tr>
      <tr className="border-b border-card-border">
        <td className="py-3 px-4 font-medium text-foreground">Encrypted Storage Adapter (client-side)</td>
        <td className="py-3 px-4 text-green-600 dark:text-green-400">Stable — <code>EncryptedStorageAdapter</code> exported from <code>@topgunbuild/client</code></td>
      </tr>
    </tbody>
  </table>
</div>

For full maturity tracking across deployment modes, see [/docs/roadmap](/docs/roadmap).
...
| **RBAC** | The server enforces per-connection, per-map permissions via `WriteValidator`. Principals are identified via JWT `sub` claims; ACLs are checked per operation. A dedicated role abstraction (role definitions and role-to-ACL bindings) is not a separate surface — see `/docs/roadmap`. |
...
<div className="bg-brand-subtle/10 border border-brand-subtle/20 rounded-xl p-6 my-6 not-prose">
  <h3 className="text-lg font-semibold text-brand mb-2">Server API Reference</h3>
  <span className="text-brand">For the full Rust server embed API including <code className="px-1.5 py-0.5 rounded bg-brand-subtle/10 text-sm">NetworkConfig</code> and <code className="px-1.5 py-0.5 rounded bg-brand-subtle/10 text-sm">TlsConfig</code>, see the <a href="/docs/reference/server" className="underline">Server API reference</a>.</span>
</div>
...
<div className="bg-brand-subtle/10 border border-brand-subtle/20 rounded-xl p-6 my-6 not-prose">
  <h3 className="text-lg font-semibold text-brand mb-2">API Reference</h3>
  <span className="text-brand">For detailed API documentation, see the <a href="/docs/reference/adapter#encryptedstorageadapter" className="underline">EncryptedStorageAdapter section</a> in the Adapter API reference.</span>
</div>
