...
DataFusion supports a broad SQL dialect. The subset relevant to TopGun's single-map usage is listed below (aggregation queries automatically execute distributed across the cluster). For the full grammar see [datafusion.apache.org](https://datafusion.apache.org/user-guide/sql/index.html).
...
When a SQL query includes `GROUP BY` or aggregate functions (`COUNT`, `SUM`, `AVG`, `MIN`, `MAX`), TopGun automatically fans the query out across all cluster nodes using a DAG (directed acyclic graph) execution pipeline. This means aggregation queries see data from the entire cluster, not just the local node.

**How it works:**

1. The coordinator node compiles the SQL into a DAG plan
2. Partition-specific scan + aggregate tasks are dispatched to the nodes that own the data
3. Each node runs its partial aggregation locally and streams results back
4. The coordinator merges partial results, then applies final `ORDER BY` and `LIMIT`

Simple `SELECT`/`WHERE` queries without aggregation still execute locally on the connected node via DataFusion.
...
<div className="bg-brand-subtle/10 border-l-4 border-brand-subtle p-4 my-4 not-prose">
  <p className="text-sm text-brand">
    For a detailed explanation of the DAG execution pipeline, partition-aware routing, and query type comparison, see the <a href="/docs/guides/distributed-queries" className="underline hover:no-underline">Distributed Queries</a> guide.
  </p>
</div>

## Limitations

<div className="grid md:grid-cols-1 gap-6 mb-8 not-prose">
  <FeatureCard
    icon={AlertTriangle}
    iconColor="text-orange-500"
...
<div className="flex justify-between pt-10 mt-10 border-t border-card-border not-prose">
  <a href="/docs/guides/distributed-queries" className="group flex flex-col gap-1 px-6 py-4 rounded-lg border border-card-border hover:border-neutral-400 dark:hover:border-neutral-700 transition-colors">
    <span className="text-xs text-neutral-500">Previous</span>
    <div className="flex items-center gap-2 text-foreground font-semibold">
      <ChevronRight className="w-4 h-4 rotate-180 group-hover:-translate-x-1 transition-transform" /> Distributed Queries
    </div>
  </a>
