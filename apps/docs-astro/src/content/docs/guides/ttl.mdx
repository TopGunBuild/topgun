...
function GameLobbies() {
  const orMap = useORMap<string, { status: string }>('games:active');
  // useORMap returns the raw ORMap<K, V> instance — call imperative methods directly
  const addLobby = (id: string, ttlMs: number) =>
    orMap.add(id, { status: 'waiting' }, ttlMs);
...
<AlertBox variant="info" title="Imperative-by-design" text="TTL is set at the imperative API layer via getMap().set(key, value, ttlMs). useMutation does not accept a TTL option argument — for React components, call useMutation(...).map.set(key, value, ttlMs) or use useClient() + client.getMap(...) directly. This is imperative-by-design: the TTL option lives on MapSetOptions at the CRDT level, below the hook surface. For everyday React UI code without TTL, see the hook-first API at /docs/quick-start." />
