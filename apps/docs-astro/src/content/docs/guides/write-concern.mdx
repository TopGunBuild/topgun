...
<AlertBox variant="warning" title="Partially Implemented" text="Write concern levels beyond the default are not yet configurable. The server currently acknowledges writes at the local level only." />

<AlertBox variant="info" title="Hook-first API for React UI code" text="Write Concern is an option at the imperative API layer — getMap().set(key, value, { writeConcern, timeout }). useMutation does not pass through a WriteOptions argument; for React components that need explicit write-concern control, use useClient() (from @topgunbuild/react) + client.getMap(...) directly. The examples below document the WriteOptions shape; for everyday UI code, prefer the hook-first API shown at /docs/quick-start." />
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 not-prose">
  <div className="p-4 bg-brand-subtle/10 rounded-lg border border-brand-subtle/20">
    <div className="flex items-center gap-2 mb-2">
      <Zap className="w-5 h-5 text-brand-subtle" />
      <h3 className="font-semibold text-brand">Low Latency</h3>
    </div>
...
<div className="space-y-4 not-prose mb-8">
  ...
  <div className="p-4 rounded-lg border border-brand-subtle/20 bg-brand-subtle/10">
    <div className="flex items-center gap-3 mb-2">
      <Database className="w-5 h-5 text-brand-subtle" />
      <h4 className="font-bold text-foreground">MEMORY</h4>
      <span className="text-xs px-2 py-0.5 bg-brand-subtle/10 rounded text-brand">Default</span>
...
<div className="space-y-4 not-prose mb-8">
  ...
  <div className="p-4 rounded-lg bg-brand-subtle/10 border border-brand-subtle/20">
    <h4 className="font-semibold text-brand mb-2">Handle Partial Success</h4>
    <p className="text-sm text-brand">
      Even if a write doesn't reach the requested level before timeout, it may have been partially processed. Check <code className="bg-brand-subtle/10 px-1 rounded">achievedLevel</code> to understand the actual state.
    </p>
  </div>
</div>
...
<div className="flex justify-between pt-10 mt-10 border-t border-card-border not-prose">
  ...
  <a href="/docs/guides/entry-processor" className="group flex flex-col items-end gap-1 px-6 py-4 rounded-lg border border-card-border hover:border-neutral-400 dark:hover:border-neutral-700 transition-colors">
    <span className="text-xs text-neutral-500">Next</span>
    <div className="flex items-center gap-2 text-foreground font-semibold">
      Entry Processor <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
    </div>
  </a>
</div>
