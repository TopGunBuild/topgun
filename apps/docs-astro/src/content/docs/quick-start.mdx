...
export const serverStartCode = `# Start the TopGun test server (Rust binary, in-memory storage)
...
const client = new TopGunClient({
  ...
  serverUrl: 'ws://localhost:8080', // TopGun Rust server
  storage: adapter
});
...
<div className="not-prose">
  <p className="text-neutral-600 dark:text-neutral-300 mb-4">
    Start the TopGun server (a Rust binary) with a PostgreSQL connection for durability.
  </p>
  <CodeBlock
    title="terminal"
    language="bash"
