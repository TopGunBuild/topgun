...
**Inside the TopGun monorepo (requires Rust toolchain + `cargo build`):**

```bash
pnpm start:server
```

> **Note on `topgun dev`:** the `dev` command starts a prebuilt `target/release/topgun-server` binary from the current working directory. The CLI is available standalone (`doctor`, `setup`, `config`, `codegen`, `docker:*`, `cluster:*`), but `dev` itself requires a prebuilt server binary. A future release will ship prebuilt binaries (see [TODO-365](https://github.com/TopGunBuild/topgun)).
