...
## 7. Explore your data visually (monorepo only)
...
**Inside the TopGun monorepo**, launch the server and the dashboard together with one command:

```bash
topgun dev --admin
```

Then open **http://localhost:5173/admin/** in your browser. When the server is running in zero-config mode (`TOPGUN_NO_AUTH=1`), the dashboard opens straight to the Dashboard screen — no login credentials required.

> **Monorepo only.** The `--admin` flag requires `apps/admin-dashboard` to be present (i.e. you have the repository checked out). Out-of-monorepo users should use one of the alternatives below.

**Alternatives for out-of-monorepo / self-host setups:**

- **Hosted demo:** [https://demo.topgun.build](https://demo.topgun.build) — connects to a shared demo server (TODO-420: a personal hosted admin is coming).
- **Docker Compose:** `docker compose --profile admin up` — starts the server and the admin dashboard at **http://localhost:3001**.
