...
export const idbAdapterCode = `import { TopGunClient } from '@topgunbuild/client';
import { IDBAdapter } from '@topgunbuild/adapters';

// No loading state needed — IDBAdapter is memory-first
...
// and flushed once IndexedDB is ready
client.getMap('todos').set('task-1', { text: 'Hello' });

// Optional: wait for IndexedDB if you need to read what you just wrote
...
export const postgresBasicCode = `# The Rust server handles PostgreSQL storage internally
# Configure via the DATABASE_URL environment variable:
DATABASE_URL=postgres://postgres:secret@localhost:5432/myapp \\
PORT=8080 \\
topgun-server

# For the Rust PostgresDataStore API, see: /docs/reference/server`;

export const postgresCustomTableCode = `# The Rust server uses its own table format (topgun_maps, topgun_merkle)
# Tables are auto-created on startup. Configure only the connection:
DATABASE_URL=postgres://user:pass@localhost/myapp topgun-server

# For custom table configuration via the Rust embed API,
# see: /docs/reference/server`;

export const postgresServerCode = `# Start the server with PostgreSQL persistence
DATABASE_URL=postgres://user:pass@localhost/myapp \\
PORT=8765 \\
topgun-server

# The Rust server automatically connects to PostgreSQL,
# creates tables, and manages the storage lifecycle.
# See: /docs/reference/server`;

export const encryptedAdapterCode = `import { TopGunClient, EncryptedStorageAdapter } from '@topgunbuild/client';
import { IDBAdapter } from '@topgunbuild/adapters';
...
<div className="bg-brand-subtle/10 border border-brand-subtle/20 rounded-xl p-4 mb-4 not-prose">
  <span className="text-brand"><strong>Note:</strong> PostgreSQL storage is handled by the <strong>Rust server</strong> (<code className="px-1.5 py-0.5 rounded bg-brand-subtle/10 text-sm">PostgresDataStore</code>), not a client adapter. Configure via <code className="px-1.5 py-0.5 rounded bg-brand-subtle/10 text-sm">DATABASE_URL</code> environment variable. See the <a href="/docs/reference/server" className="underline">Server API reference</a> for details.</span>
</div>

<div className="flex items-center gap-2 mb-3 not-prose">
  <Database className="w-5 h-5 text-brand-subtle" />
...
<div className="not-prose">
  <CodeBlock title="PostgreSQL Setup" language="bash" code={postgresBasicCode} />
</div>
...
<div className="not-prose">
  <CodeBlock title="Custom Table Configuration" language="bash" code={postgresCustomTableCode} />
</div>
...
<div className="not-prose">
  <CodeBlock title="Server with PostgreSQL" language="bash" code={postgresServerCode} />
</div>
...
<div className="bg-brand-subtle/10 border border-brand-subtle/20 rounded-xl p-6 mb-6 not-prose">
  <h3 className="text-lg font-semibold text-brand mb-2">Learn More</h3>
  <span className="text-brand">For detailed security considerations and best practices, see the <a href="/docs/guides/security#client-side-encryption" className="underline">Client-Side Encryption section</a> in the Security Guide.</span>
</div>

<div className="flex justify-between pt-10 mt-10 border-t border-card-border not-prose">
  <a href="/docs/reference/cli" className="group flex flex-col gap-1 px-6 py-4 rounded-lg border border-card-border hover:border-neutral-400 dark:hover:border-neutral-700 transition-colors">
    <span className="text-xs text-neutral-500">Previous</span>
    <div className="flex items-center gap-2 text-foreground font-semibold">
      <ChevronRight className="w-4 h-4 rotate-180 group-hover:-translate-x-1 transition-transform" /> CLI Reference
    </div>
  </a>
