...
Returns a live query subscription. The handle emits an initial result and delta updates.
...
Returns a pub/sub topic for ephemeral fan-out messaging (not CRDT-persisted).
...
Returns a distributed lock handle.
...
Returns a PN-Counter (positive-negative) for offline-capable increment/decrement.
