...
### `executeOnKey(mapName, key, processor)`
...
Atomic read-modify-write on the server. Eliminates client-side races.
...
### `executeOnKeys(mapName, keys, processor)`
...
Apply a processor across many keys atomically per key.
...
const results = await client.executeOnKeys(
  ...
  {
    name: 'reset',
    code: `return { value: 0, result: value };`,
  }
);
```
...
Register custom merge resolvers (first-write-wins, numeric constraints, owner-only edits) and observe merge rejections.
