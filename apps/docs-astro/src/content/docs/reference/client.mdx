...
export const singleServerCode = `import { TopGunClient } from '@topgunbuild/client';
...
export const clusterModeCode = `import { TopGunClient } from '@topgunbuild/client';
import { IDBAdapter } from '@topgunbuild/adapters';
...
export const httpSyncModeCode = `import { HttpSyncProvider, SyncEngine } from '@topgunbuild/client';
...
export const autoConnectionModeCode = `import { AutoConnectionProvider, SyncEngine } from '@topgunbuild/client';
import { HLC } from '@topgunbuild/core';
import { IDBAdapter } from '@topgunbuild/adapters';
...
<AlertBox variant="info" title="Hook-first API for React UI code" text="For React UI code, prefer the hook-first API: useQuery<Todo>('todos') for reads, useMutation<Todo>('todos') for writes (see /docs/quick-start for the canonical pattern). This reference page documents the lower-level TopGunClient API surface used inside hooks and in non-React contexts (Node.js, service workers, testing)." />
...
<div className="bg-brand-subtle/10 border border-brand-subtle/20 rounded-xl p-6 my-6 not-prose">
  <div className="flex items-center gap-2 mb-2">
    ...
    <h4 className="font-semibold text-brand">Smart Routing</h4>
  </div>
  <span className="text-brand">
    When <code className="px-1.5 py-0.5 rounded bg-brand-subtle/10 text-sm">smartRouting</code> is enabled,
...
<div className="not-prose">
  <CodeBlock language="typescript" code={httpSyncModeCode} />
</div>

#### HttpSyncProviderConfig

<div className="not-prose">
  <ApiConstructor signature="interface HttpSyncProviderConfig">
    <ApiParam
      name="url"
      type="string"
      description="HTTP URL of the TopGun server (e.g., 'https://api.example.com')."
    />
    <ApiParam
      name="clientId"
      type="string"
      description="Client identifier for request construction."
    />
    <ApiParam
      name="hlc"
      type="HLC"
      description="Hybrid Logical Clock instance for causality tracking."
    />
    <ApiParam
      name="authToken?"
      type="string"
      description="JWT auth token for the Authorization header."
    />
    <ApiParam
      name="pollIntervalMs?"
      type="number"
    ...
    <ApiParam
      name="syncMaps?"
      type="string[]"
      description="Map names to sync deltas for on each poll."
    />
    <ApiParam
      name="fetchImpl?"
      type="typeof fetch"
      description="Custom fetch implementation for testing or platform compatibility."
    />
  </ApiConstructor>
</div>
...
<div className="not-prose">
  <CodeBlock language="typescript" code={autoConnectionModeCode} />
</div>

#### AutoConnectionProviderConfig

<div className="not-prose">
  <ApiConstructor signature="interface AutoConnectionProviderConfig">
    <ApiParam
      name="url"
      type="string"
      description="Server URL (ws:// or http://)."
    />
    <ApiParam
      name="clientId"
      type="string"
    ...
    <ApiParam
      name="hlc"
      type="HLC"
      description="Hybrid Logical Clock instance."
    />
    <ApiParam
      name="maxWsAttempts?"
      type="number"
      description="Maximum WebSocket connection attempts before falling back to HTTP. Default: 3"
    />
    <ApiParam
      name="authToken?"
      type="string"
    ...
    <ApiParam
      name="httpOnly?"
      type="boolean"
      description="Skip WebSocket and use HTTP-only mode. Default: false"
    />
    <ApiParam
      name="httpPollIntervalMs?"
      type="number"
      description="HTTP polling interval in milliseconds. Default: 5000"
    />
    <ApiParam
      name="syncMaps?"
      type="string[]"
      description="Map names to sync via HTTP."
    />
    <ApiParam
      name="fetchImpl?"
      type="typeof fetch"
      description="Custom fetch implementation for HTTP mode."
    />
  </ApiConstructor>
</div>
...
<div className="bg-brand-subtle/10 border border-brand-subtle/20 rounded-xl p-6 my-6 not-prose">
  <div className="flex items-center gap-2 mb-2">
    <Network className="w-5 h-5 text-brand" />
    <h4 className="font-semibold text-brand">HTTP Mode Limitations</h4>
  </div>
  <span className="text-brand">
...
<div className="bg-brand-subtle/10 border border-brand-subtle/20 rounded-xl p-6 my-6 not-prose">
  <div className="flex items-center gap-2 mb-2">
    ...
    <h4 className="font-semibold text-brand">Entry Processor</h4>
  </div>
  <span className="text-brand">
