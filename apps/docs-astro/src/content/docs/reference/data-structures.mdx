...
<div className="bg-brand-subtle/10 border-l-4 border-brand-subtle p-4 my-6 not-prose">
  <p className="text-sm text-brand">
    <strong>Which one to use?</strong> Use <code className="bg-brand-subtle/10 px-1 rounded">LWWMap</code> for key-value data where each key has one value (profiles, settings).
    Use <code className="bg-brand-subtle/10 px-1 rounded">ORMap</code> for collections where a key can have multiple values (todo lists, tags, comments).
...
<div className="flex items-center gap-2 mb-3 not-prose">
  <Eye className="w-5 h-5 text-brand-subtle" />
