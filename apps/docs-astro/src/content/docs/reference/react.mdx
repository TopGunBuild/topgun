...
Escape hatch returning the underlying `TopGunClient`. Use this for imperative operations (calling `client.executeOnKey`, `client.getEventJournal`, etc.) inside event handlers and effects.
...
Atomic read-modify-write via a server-side entry processor.
...
function IncrementButton() {
  const { execute, pending } = useEntryProcessor('stats', 'pageViews');

  return (
    <button
      disabled={pending}
      onClick={() => execute(BuiltInProcessors.INCREMENT(1))}
    >
      Increment
    </button>
  );
...
```typescript
function useEntryProcessor<V = unknown, R = V>(
  mapName: string,
  key: string,
): { execute: (def: EntryProcessorDef<V, R>) => Promise<EntryProcessorResult<R>>; pending: boolean; lastResult?: EntryProcessorResult<R> }
```
...
Surface merge rejections (the server's conflict resolver rejected a remote change). Useful for telling the user "your edit conflicted".
...
Register a custom conflict resolver scoped to a component lifecycle. Unregisters on unmount.

```typescript
function useConflictResolver(
  mapName: string,
  def: ConflictResolverDef,
): { registered: boolean; error: Error | null }
```
