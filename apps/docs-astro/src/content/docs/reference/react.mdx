...
`@topgunbuild/react` ships hooks that wrap the imperative `TopGunClient` API. Every hook below resolves the client via `<TopGunProvider client={client}>`, which is the only required ancestor.
...
Live query subscription. Returns `data`, `loading`, `error`, change-tracking fields, pagination info, and per-record sync state.
...
Returns the current `RecordSyncState` for a `(mapName, key)` pair. The state is one of `synced`, `pending`, `conflict`, or `offline`. Re-renders only when *that* key's state changes.
...
Subscribe to (and publish on) an ephemeral pub/sub topic.
