---
title: Server API
description: The Server API manages the TopGun backend including client connections, synchronization, clustering, and security.
order: 22
---
...
export const readyCode = `const server = ServerFactory.create(config);
await server.ready();
...
The `ServerFactory.create()` method creates a server instance that manages the TopGun backend. It handles client connections,
