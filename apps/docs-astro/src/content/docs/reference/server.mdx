...
The repository entrypoint is `pnpm start:server`, which wraps `cargo run --bin test-server --release`. The binary is named `test-server` because the build was originally produced for the integration test suite; it is the same binary used in production-style demos like `demo.topgun.build`.
...
The `test-server` binary exposes five `clap`-derived flags (defined in `packages/server-rust/src/bin/test_server.rs`):

| Flag | Type | Default | Description |
|---|---|---|---|
| `--node-id <ID>` | string | `"test-server-node"` | Unique identifier for this node in a cluster. |
...
|---|---|---|---|---|
| `STORAGE_BACKEND` | string | `"redb"` | `bin/test_server.rs:109` | One of `redb`, `postgres`, `null`. `null` is in-memory ephemeral (test only). |
| `TOPGUN_REDB_PATH` | path | `./topgun.redb` | `bin/test_server.rs:115` | File path for the embedded redb database (used when `STORAGE_BACKEND=redb`). |
| `DATABASE_URL` | string | none | `bin/test_server.rs:126`, `storage/datastores/postgres.rs:693` | Postgres connection string (required when `STORAGE_BACKEND=postgres`). |

### Networking

| Variable | Type | Default | Consumer | Notes |
|---|---|---|---|---|
| `PORT` | u16 | `8080` | `bin/test_server.rs:168` (via `#[arg(env = "PORT")]`) | Client WebSocket port. Equivalent to `--port`. |
| `TOPGUN_BIND_ADDR` | string | `"0.0.0.0"` | `bin/test_server.rs:203` | Bind address for the client WebSocket listener. Override to `127.0.0.1` for loopback-only deployments. |
...
| `TOPGUN_NO_AUTH` | truthy | unset | `bin/test_server.rs:477` | If set (`"true"`, `"1"`), disables auth. Test and demo use only. |
| `INSECURE_FORWARD_AUTH_ERRORS` | truthy | unset | `bin/test_server.rs:470` | If set, forwards upstream auth errors verbatim in responses. Insecure; do not enable in production. |
...
| `TOPGUN_ADMIN_DIR` | path | `./admin-dashboard/dist` | `network/module.rs:582` | Static-file directory served at `/admin/*`. |

### Search indexes

| Variable | Type | Default | Consumer | Notes |
|---|---|---|---|---|
| `TOPGUN_INDEX_PATH` | path | none | `bin/test_server.rs:516`, `network/module.rs:316`, `network/handlers/admin.rs:1078` | Directory holding the BM25 full-text-search index. |
...
| Variable | Type | Default | Consumer | Notes |
|---|---|---|---|---|
| `RUST_LOG` | filter expression | `"info"` | `bin/test_server.rs:191` (via `tracing_subscriber::EnvFilter::try_from_default_env()`) | Standard tracing filter (e.g., `topgun_server=debug,info`). Not read via `std::env::var` directly. |
...
struct NetworkConfig {
    ...
    cors_origins: Vec<String>,       // default ["*"]
