...
import { KeyPairEd25519 } from '@topgunbuild/crypto';
...
export const createKey = async (username: string): Promise<KeyPairEd25519> =>
{
    ...
    return navigator.credentials.create({ publicKey })
        .then(async (res) =>
        {
            ...
            return KeyPairEd25519.fromString(baseEncode(new Uint8Array(Buffer.concat([Buffer.from(secretKey), Buffer.from(pubKey)]))));
        });
};

// Ecrecover returns two possible public keys for a given signature
export const getKeys = async (username: string): Promise<[KeyPairEd25519, KeyPairEd25519]> =>
{
    const cleanUserName = validateUsername(username);
    if (!f2l.f2l)
    {
    ...
    return navigator.credentials.get({ publicKey })
        .then(async (response) =>
        {
            ...
            const firstKeyPair   = KeyPairEd25519.fromString(baseEncode(new Uint8Array(Buffer.concat([Buffer.from(firstEDSecret), Buffer.from(firstEDPublic)]))));
            const secondKeyPair  = KeyPairEd25519.fromString(baseEncode(new Uint8Array(Buffer.concat([Buffer.from(secondEDSecret), Buffer.from(secondEDPublic)]))));
