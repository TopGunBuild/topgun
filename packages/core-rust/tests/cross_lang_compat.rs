...
use topgun_core::hlc::{ClockSource, ORMapRecord, Timestamp, HLC};
...
fn build_test_records() -> [ORMapRecord<Value>; 3] {
    let mut user_data = BTreeMap::new();
    user_data.insert("name".to_string(), Value::String("Alice".to_string()));
...
#[test]
fn ormap_merkle_hash_deterministic_across_insertion_orders() {
    // Three ORMaps receive the same records applied in different orders.
    // Their Merkle root hashes must all be identical.
    let mut map_a = make_test_map("test-a");
    let mut map_b = make_test_map("test-b");
    let mut map_c = make_test_map("test-c");

    let [r1, r2, r3] = build_test_records();

    // Order 1: r1, r2, r3
    map_a.apply("user:1", r1.clone());
    map_a.apply("user:1", r2.clone());
    map_a.apply("user:1", r3.clone());

    // Order 2: r3, r2, r1
    map_b.apply("user:1", r3.clone());
    map_b.apply("user:1", r2.clone());
    map_b.apply("user:1", r1.clone());

    // Order 3: r2, r1, r3
    map_c.apply("user:1", r2);
    map_c.apply("user:1", r1);
    map_c.apply("user:1", r3);

    let root_a = map_a.merkle_tree().get_root_hash();
    let root_b = map_b.merkle_tree().get_root_hash();
    let root_c = map_c.merkle_tree().get_root_hash();

    assert_ne!(root_a, 0, "Merkle root should be non-zero after inserts");
    assert_eq!(
        root_a, root_b,
        "Merkle root must match regardless of insertion order (a vs b)"
    );
    assert_eq!(
        root_a, root_c,
        "Merkle root must be stable across independent ORMap instances (a vs c)"
    );
}
