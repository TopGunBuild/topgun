...
export class IndexedLWWMap<K extends string, V> extends LWWMap<K, V> {
      ...
      case 'full-scan': {
        ...
        if (fallback) {
          ...
          return fallback.retrieve(
            step.predicate as unknown as IndexQuery<unknown>,
          ) as ResultSet<K>;
        }
  ...
  public keys(): Iterable<K> {
    // Generator functions inside object literals get their own `this` binding,
    // so we capture the outer `this` reference before entering the generator.
