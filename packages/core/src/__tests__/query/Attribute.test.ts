...
describe("Attribute edge cases", () => {
  ...
  it("should handle numeric values", () => {
    const numAttr = simpleAttribute<{ num: number }, number>(
      "num",
      (r) => r.num,
    );
  ...
  it("should handle boolean values", () => {
    const boolAttr = simpleAttribute<{ active: boolean }, boolean>(
      "active",
      (r) => r.active,
    );
  ...
  it("should handle nested property extraction", () => {
    ...
    expect(nestedAttr.getValue({ user: { profile: { name: "John" } } })).toBe(
      "John",
    );
  });
});
