...
describe("QueryExecutor", () => {
  ...
  describe("execute() with FTS queries", () => {
    ...
    it("should include matched terms in results", () => {
      ...
      const withTerms = results.filter(
  ...
  describe("execute() with range queries", () => {
    ...
    it("should execute between query", () => {
      ...
      expect(
        results.every((r) => r.value.price >= 30 && r.value.price < 50),
      ).toBe(true);
    });
  });
  ...
  describe("execute() with logical queries", () => {
    it("should execute AND query", () => {
      ...
      expect(
        results.every(
          (r) => r.value.status === "published" && r.value.category === "tech",
        ),
      ).toBe(true);
    });
  ...
  describe("applyOrdering()", () => {
    ...
    it("should sort by field value", () => {
      ...
      for (let i = 1; i < results.length; i++) {
        expect(results[i - 1].value.price).toBeLessThanOrEqual(
          results[i].value.price,
        );
      }
    });
  ...
  describe("limit and offset", () => {
    ...
    it("should apply offset", () => {
      ...
      const limitedResults = executor.execute(query, data, {
        limit: allResults.length - 1,
      });
  ...
  describe("edge cases", () => {
    ...
    it("should handle FTS query with no index", () => {
      ...
      const noFtsOptimizer = new QueryOptimizer<string, TestDocument>({
        indexRegistry,
      });
