...
#   --live-census-interval 0                        (disarmed, a literal)
...
#   diff spec355-width.sh spec362-durable.sh
#
...
#   SPEC362_DATA_DIR              data dir for this run (default: target/)
#   SPEC362_OUT_DIR               artifact dir     (default: this script's dir)
#   SPEC362_SOAK_BIN              prebuilt soak_harness bench binary
#   SPEC362_SMOKE_DURATION        SMOKE ONLY: override --duration (seconds)
#   SPEC362_SMOKE_SAMPLE_INTERVAL SMOKE ONLY: override the CSV cadence
#   SPEC362_FORCE=1               overwrite pre-existing artifacts
...
usage() {
  cat >&2 <<'EOF'
usage: spec362-durable.sh <cell>

  THE DECIDING CELL -- frozen here, EXECUTED BY SPEC-362b, NOT by this spec:
    long4h     HEAD server @ width 1000, 14400s, log directive ARMED
...
case "$CELL" in
  # The deciding cell. FROZEN HERE, EXECUTED BY SPEC-362b: this spec holds no
  # 4h cell of its own, and running it from here would create exactly the new
  # measurement lineage the spec forbids itself.
  long4h)     WIDTH="";  DURATION=14400; SAMPLE_INTERVAL=60; PROVENANCE=no
              CELL_CRASH_INTERVAL=0; ARM_LOG=yes
              EXTRA_FLAGS=""; BASE="spec362-long4h" ;;
  ...
  crashctl)   WIDTH="";  DURATION=900;   SAMPLE_INTERVAL=60; PROVENANCE=no
              ...
              EXTRA_FLAGS=""; BASE="spec362-crashctl" ;;
  *)         usage ;;
esac
...
OUT_DIR="${SPEC362_OUT_DIR:-$EVIDENCE_DIR}"
...
DATA_DIR="${SPEC362_DATA_DIR:-${REPO_ROOT}/target/spec362-${CELL}-data}"
...
# ---------------------------------------------------------------------------
SMOKE=0
if [ -n "${SPEC362_SMOKE_DURATION:-}" ]; then
  SMOKE=1
  DURATION="$SPEC362_SMOKE_DURATION"
  if [ -n "${SPEC362_SMOKE_SAMPLE_INTERVAL:-}" ]; then
    SAMPLE_INTERVAL="$SPEC362_SMOKE_SAMPLE_INTERVAL"
  fi
  echo "############################################################"
  ...
  if [ "$OUT_DIR" = "$EVIDENCE_DIR" ]; then
    ...
    echo "  $EVIDENCE_DIR" >&2
    echo "Set SPEC362_OUT_DIR to a scratch directory." >&2
    exit 2
  fi
elif [ -n "${SPEC362_SMOKE_SAMPLE_INTERVAL:-}" ]; then
  echo "WARNING: SPEC362_SMOKE_SAMPLE_INTERVAL ignored outside smoke mode;" >&2
...
for f in "$CSV_OUT" "$JSON_OUT" "$PROGRESS_OUT" "$MECH_OUT" "$MECH_RAW" "$DURABLE_OUT" "$MATRIX_OUT" "$CONSOLE_OUT"; do
  if [ -e "$f" ] && [ "${SPEC362_FORCE:-0}" != "1" ]; then
    echo "FATAL: artifact already exists: $f" >&2
    echo "       Move it aside, or re-run with SPEC362_FORCE=1 to overwrite." >&2
    exit 1
  fi
done
...
{
  echo
  echo "=== spec362 durable run: cell ${CELL} ==="
  ...
  echo "  --live-census-interval: 0 (DISARMED)"
  ...
  if [ "$PROVENANCE" = "yes" ]; then
    ...
    echo "  pinned SHA:            ${SPEC362_PIN_SHA:-<record it here: export SPEC362_PIN_SHA>}"
    echo "  pin resolved by:       ${SPEC362_PIN_CMD:-<record it here: export SPEC362_PIN_CMD>}"
    echo "  worktree path:         ${SPEC362_PIN_WORKTREE:-<record it here: export SPEC362_PIN_WORKTREE>}"
  fi
