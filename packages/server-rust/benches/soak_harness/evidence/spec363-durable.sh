...
#   --live-census-interval PER-CELL literal: 0 on logctl-on, 0 on logctl-off
#                        and 300 on crashctl ALONE (see difference (c) below;
#                        the parent passed the literal 0 on every cell)
#   --sampler-jitter-seed                           (JITTER_SEED, echoed into
#                                                    matrix.txt from that same
#                                                    shell variable)
#   --duration           per-cell literal           (per-cell literal below)
#   --crash-interval     crashctl ONLY: an INSTRUMENT-BRANCH parameter of that
#                        control, not a departure   (per-cell literal below)
...
# instead, on the deciding cell and on the ARMED pricing arm. It is declared,
# not absorbed, and it is priced by the logctl-on / logctl-off pair.
#
...
#   diff spec355-width.sh spec363-durable.sh
#
# DERIVATIVE OF spec362-durable.sh, WHICH IS NOT EDITED. The parent spec
# declared its own runner a CLOSED diff, and the instrument has since been
# re-pinned: the witnesses must be produced by the NEW instrument, so this is a
# new file carrying EXACTLY four enumerated differences against that parent and
# no others:
#
#   (a) artifact prefix spec363-* (and, with it, the SPEC363_* env overrides
#       and the run's data dir), so the old and the new witnesses stay
#       trivially separable in evidence/;
#   (b) THE SOAK BENCH BINARY IS BUILT FROM HEAD BY THIS RUNNER, the run
#       refuses to start on a dirty .rs working tree, and matrix.txt records
#       the "soak binary commit:" and the branch point the new witnesses are
#       diffed from. The parent mtime-selected whatever deps/soak_harness-*
#       happened to be newest, which cannot prove the artifact was produced by
#       the pin it is filed under -- the provenance weakness recorded as
#       manifest P6 item 10;
#   (c) the live-copy census sampler's cadence -- the flag named in the
#       enumerated list at the top of this header -- is a PER-CELL literal
#       (0 / 0 / 300) instead of the parent's flat 0, so that producer is
#       EXECUTED at least once under this pin instead of being argued by
#       construction. It is armed on crashctl ALONE, which prices nothing, so
#       the pricing pair keeps the parent's sampler set exactly;
#   (d) the 4h deciding cell is DELETED rather than merely un-invoked; an
#       explicit refusal comment stands in its place in the cell table.
#
# NO ARM'S DURATION, CRASH INTERVAL OR RATE-/SHAPE-DETERMINING LITERAL CHANGES.
# All three arms stay at 900s, so no fourth control and no combined-budget
# allowance is created. The departure is enumerable with:
#   diff spec362-durable.sh spec363-durable.sh
#
...
#   SPEC363_DATA_DIR              data dir for this run (default: target/)
#   SPEC363_OUT_DIR               artifact dir     (default: this script's dir)
#   SPEC363_SOAK_BIN              prebuilt soak_harness bench binary
#   SPEC363_SMOKE_DURATION        SMOKE ONLY: override --duration (seconds)
#   SPEC363_SMOKE_SAMPLE_INTERVAL SMOKE ONLY: override the CSV cadence
#   SPEC363_FORCE=1               overwrite pre-existing artifacts
...
usage() {
  cat >&2 <<'EOF'
usage: spec363-durable.sh <cell>

  This runner ships NO 4h deciding cell, and cannot be made to run one: the
  cell table has no label for it. See the refusal comment in the cell table.

  The three INSTRUMENT controls (each <= 900s individually; there is no
  combined-budget allowance and no fourth control). Their verdicts are about
  the INSTRUMENT and may NEVER be quoted about the plateau:
    logctl-on  900s, crash-interval 0, log directive ARMED
               -- pricing arm A + the arming witness's positive direction
                  (matched > 0, unparsed == 0, dropped == 0)
    logctl-off 900s, crash-interval 0, log directive UNSET
               -- pricing arm B + the arming witness's negative direction
                  (matched == 0, reading INDETERMINATE_INSTRUMENT)
    crashctl   900s, crash-interval 300, log directive ARMED
               -- the DOUBLE witness: the census's Checkpoint copy-then-scan
                  branch and the origin reading's restarts > 0 guard, neither
                  of which the deciding cell's crash-interval 0 ever reaches.

  The pricing pair is honestly weak and is pre-registered as such: with n = 1
  per arm it has NO inferential power against a small perturbation; it can
  only surface a gross one. It bounds a SHIFT IN THE LEVEL of RSS, not a
...
#                 that sampler DISARMED, as every cell of the parent runner did
#                 (see difference (c) in the header block above)
#   armlog     -- yes|no: SOAK_SERVER_LOG carries the removal target's directive
#   extra      -- extra harness flags
#   base       -- artifact basename
case "$CELL" in
  # THE 4h DECIDING CELL IS DELETED, DELIBERATELY AND WITH ITS REASON WRITTEN
  # DOWN. The parent runner carried a `long4h` label here and relied on nobody
  # typing it. That cell is OUT OF SCOPE for this spec twice over: it is the
  # successor spec's to execute, and running a 14400s cell from here would
  # breach the same section's X8 discipline that permits this spec only <= 900s
  # controls with no fourth control, no width-1000 deciding matrix and no new
  # measurement lineage. A forbidden cell that is one keystroke away in a
  # runner this spec ships is a hazard; deleting the label converts it into a
  # documented refusal. Asking for it falls through to `usage` and exits 2.
  # The two pricing arms. Same binary, same matrix, 900s each; the ONLY
  # difference between them is whether the log directive is armed, which is
  # what makes the pair a price for the one declared departure AND the
  # two-directional arming witness.
  logctl-on)  WIDTH="";  DURATION=900;   SAMPLE_INTERVAL=60; PROVENANCE=no
              CELL_CRASH_INTERVAL=0; ARM_LOG=yes
              # --live-census-interval 0: DISARMED, the parent's sampler set
              # verbatim, which is what keeps this pricing arm uncontaminated.
              CELL_LIVE_CENSUS=0
              EXTRA_FLAGS=""; BASE="spec363-logctl-on" ;;
  logctl-off) WIDTH="";  DURATION=900;   SAMPLE_INTERVAL=60; PROVENANCE=no
              CELL_CRASH_INTERVAL=0; ARM_LOG=no
              # --live-census-interval 0: DISARMED, for the same reason as its
              # armed twin -- the pair must differ in the log directive alone.
              CELL_LIVE_CENSUS=0
              EXTRA_FLAGS=""; BASE="spec363-logctl-off" ;;
  # The crash control. Its non-zero crash interval is an INSTRUMENT-BRANCH
  # parameter of this control, NOT a departure: it exists solely to execute
  # the census's Checkpoint path and the origin reading's restart guard, it is
  # never applied to the deciding cell, and it prices nothing.
  crashctl)   WIDTH="";  DURATION=900;   SAMPLE_INTERVAL=60; PROVENANCE=no
              CELL_CRASH_INTERVAL=300; ARM_LOG=yes
              # --live-census-interval 300: ARMED on this control ALONE. The
              # live-copy census producer is the site of the copy-completion
              # fix, and no committed arm has ever executed it. This arm prices
              # nothing, so arming the sampler here costs the pricing pair
              # nothing; it adds no minute of budget, no fourth control and no
              # new lineage. Its 900s duration and 300s crash interval are
              # unchanged, and the first live copy at ~300s can be consumed by
              # the crash/restart window: one live row is the pre-registered
              # PASS, two is the nominal schedule, zero is a recorded finding.
              CELL_LIVE_CENSUS=300
              EXTRA_FLAGS=""; BASE="spec363-crashctl" ;;
  *)         usage ;;
esac
...
OUT_DIR="${SPEC363_OUT_DIR:-$EVIDENCE_DIR}"
...
DATA_DIR="${SPEC363_DATA_DIR:-${REPO_ROOT}/target/spec363-${CELL}-data}"
...
# ---------------------------------------------------------------------------
SMOKE=0
if [ -n "${SPEC363_SMOKE_DURATION:-}" ]; then
  SMOKE=1
  DURATION="$SPEC363_SMOKE_DURATION"
  if [ -n "${SPEC363_SMOKE_SAMPLE_INTERVAL:-}" ]; then
    SAMPLE_INTERVAL="$SPEC363_SMOKE_SAMPLE_INTERVAL"
  fi
  echo "############################################################"
  ...
  if [ "$OUT_DIR" = "$EVIDENCE_DIR" ]; then
    ...
    echo "  $EVIDENCE_DIR" >&2
    echo "Set SPEC363_OUT_DIR to a scratch directory." >&2
    exit 2
  fi
elif [ -n "${SPEC363_SMOKE_SAMPLE_INTERVAL:-}" ]; then
  echo "WARNING: SPEC363_SMOKE_SAMPLE_INTERVAL ignored outside smoke mode;" >&2
...
# Per-cell: 0 on the deciding cell and on both pricing arms (0 parses to None:
# NO kill -9 during the run), non-zero on crashctl ALONE, where it is an
# instrument-branch parameter of that control rather than a matrix change.
...
# The branch point these artifacts' .rs diff is read from. A literal, because
# `git merge-base origin/main HEAD` moves as origin/main moves, and a symbol an
# acceptance check compares against must not drift after the run.
SPEC363_BASE_COMMIT=2870862d4892368dd03238e56790f7bdd5779d6e
...
if [ -n "${SPEC363_SOAK_BIN:-}" ]; then
  echo "WARNING: SPEC363_SOAK_BIN is set, so this runner did NOT build the bench" >&2
...
if [ -n "$SOAK_BIN" ]; then
  ...
  SOAK_BIN_COMMIT="<UNPROVEN: SPEC363_SOAK_BIN override, not built by this run>"
else
...
for f in "$CSV_OUT" "$JSON_OUT" "$PROGRESS_OUT" "$MECH_OUT" "$MECH_RAW" "$DURABLE_OUT" "$MATRIX_OUT" "$CONSOLE_OUT"; do
  if [ -e "$f" ] && [ "${SPEC363_FORCE:-0}" != "1" ]; then
    echo "FATAL: artifact already exists: $f" >&2
    echo "       Move it aside, or re-run with SPEC363_FORCE=1 to overwrite." >&2
    exit 1
  fi
done
...
{
  echo
  echo "=== spec363 durable run: cell ${CELL} ==="
  ...
  echo "  branch point:       ${SPEC363_BASE_COMMIT}"
  echo "    built:        $(date -r "$SOAK_BIN" -u '+%Y-%m-%dT%H:%M:%SZ')"
  echo "  server binary:  $SERVER_BIN"
  echo "    built:        $(date -r "$SERVER_BIN" -u '+%Y-%m-%dT%H:%M:%SZ')"
  ...
  if [ "$PROVENANCE" = "yes" ]; then
    ...
    echo "  pinned SHA:            ${SPEC363_PIN_SHA:-<record it here: export SPEC363_PIN_SHA>}"
    echo "  pin resolved by:       ${SPEC363_PIN_CMD:-<record it here: export SPEC363_PIN_CMD>}"
    echo "  worktree path:         ${SPEC363_PIN_WORKTREE:-<record it here: export SPEC363_PIN_WORKTREE>}"
  fi
  echo "  --- pinned matrix (identical to spec355-width.sh; crash-interval is"
  echo "      per-cell and is non-zero on crashctl ALONE) ---"
