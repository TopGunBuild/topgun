...
usage() {
  cat >&2 <<'EOF'
usage: spec372-allocdiag.sh <cell>

  A COPY of spec371-memdiag.sh, which is not edited. The difference list
  ...
  300s, log directive ARMED, SIGKILL teardown:
    s1a s1b  flavour SYS (plain release, System allocator)  900s
    j1a j1b  flavour JE  (alloc-jemalloc)                   900s
    m1a m1b  flavour MI  (alloc-mimalloc)                   900s
    k1       flavour CA  (count-alloc, churn instrument)    900s
    s2       flavour SYS                                    14400s
    j2       flavour JE                                     14400s
    m2       flavour MI                                     14400s
    a2j      flavour SPEC372_A2J_FLAVOUR (JE|MI), 900s, journal OFF
  Every other matrix literal is spec371-memdiag.sh's own.

  Any other argument falls through to this text and exits 2.

  The run REFUSES TO START, before any clock, if
  SPEC372_CODE_FREEZE still reads its own placeholder value, or unless the
  ...
  SPEC372_HARNESS_BIN and SPEC372_CHAIN_START_EPOCH are REQUIRED, and are
  exported by spec372-chain1.sh / spec372-chain2.sh. This runner builds
  nothing.
EOF
  exit 2
}
...
#   flavour    -- SYS|JE|MI|CA: the server build the cell must launch
...
case "$CELL" in
  s1a|s1b) DURATION=900;   FLAVOUR=SYS; CELL_JOURNAL=default; CELL_GRACEFUL=no ;;
  j1a|j1b) DURATION=900;   FLAVOUR=JE;  CELL_JOURNAL=default; CELL_GRACEFUL=no ;;
  m1a|m1b) DURATION=900;   FLAVOUR=MI;  CELL_JOURNAL=default; CELL_GRACEFUL=no ;;
  k1)      DURATION=900;   FLAVOUR=CA;  CELL_JOURNAL=default; CELL_GRACEFUL=no ;;
  s2)      DURATION=14400; FLAVOUR=SYS; CELL_JOURNAL=default; CELL_GRACEFUL=no ;;
  j2)      DURATION=14400; FLAVOUR=JE;  CELL_JOURNAL=default; CELL_GRACEFUL=no ;;
  m2)      DURATION=14400; FLAVOUR=MI;  CELL_JOURNAL=default; CELL_GRACEFUL=no ;;
  a2j)
    # The journal-off cell runs on whichever arm Stage 1 ranked first; the
    # chain reads that from the frozen Stage-1 readings and passes it here.
    case "${SPEC372_A2J_FLAVOUR:-}" in
      JE|MI) FLAVOUR="$SPEC372_A2J_FLAVOUR" ;;
      *) echo "FATAL: cell a2j needs SPEC372_A2J_FLAVOUR=JE|MI (got '${SPEC372_A2J_FLAVOUR:-}')" >&2; exit 2 ;;
    esac
    DURATION=900; CELL_JOURNAL=off; CELL_GRACEFUL=no ;;
  *)    usage ;;
esac
BASE="spec372-${CELL}"
...
DATA_DIR="${SPEC365_DATA_DIR:-${REPO_ROOT}/target/spec372-${CELL}-data}"
...
SERVER_PORT=47358       # fixed, distinct from spec362b-durable.sh's 47355 and
                         # spec371-memdiag.sh's 47357 so no two runners' cells
                         # collide on one host
...
# ---------------------------------------------------------------------------
SPEC372_CODE_FREEZE=08cdef2f
if [ "$SPEC372_CODE_FREEZE" = "PENDING_SPEC372_CODE_FREEZE" ]; then
  echo "FATAL: SPEC372_CODE_FREEZE still reads its placeholder value." >&2
...
if ! FREEZE_RS_DIFF="$(git -C "$REPO_ROOT" diff --stat "$SPEC372_CODE_FREEZE"..HEAD -- '*.rs' 2>&1)"; then
  echo "FATAL: the .rs diff against the freeze commit ${SPEC372_CODE_FREEZE} could" >&2
...
if [ -n "$FREEZE_RS_DIFF" ]; then
  echo "FATAL: the .rs tree at HEAD differs from the freeze commit ${SPEC372_CODE_FREEZE}; this run would not be filed under it" >&2
  printf '%s\n' "$FREEZE_RS_DIFF" >&2
  exit 1
fi
...
# This runner builds nothing: spec372-chain1.sh/-chain2.sh build every flavour once, into
# its own target dir, and hands the paths over. Both are required.
if [ -z "${SPEC372_HARNESS_BIN:-}" ] || [ ! -x "${SPEC372_HARNESS_BIN}" ]; then
  echo "FATAL: SPEC372_HARNESS_BIN must name the chain-built soak_harness binary." >&2
  exit 3
fi
case "${SPEC372_CHAIN_START_EPOCH:-}" in
  ''|*[!0-9]*)
    echo "FATAL: SPEC372_CHAIN_START_EPOCH must be the chain's integer start epoch." >&2
    exit 3
    ;;
esac
...
if [ "$SERVER_MTIME_EPOCH" -lt "$SPEC372_CHAIN_START_EPOCH" ]; then
...
if [ "$(date -r "$SOAK_BIN" '+%s')" -lt "$SPEC372_CHAIN_START_EPOCH" ]; then
...
{
  echo
  echo "=== spec372 allocator-evaluation run: cell ${CELL} (flavour ${FLAVOUR}) ==="
  ...
  echo "  chain start:    ${SPEC372_CHAIN_START_EPOCH}"
  ...
  echo "  code freeze:            ${SPEC372_CODE_FREEZE}"
