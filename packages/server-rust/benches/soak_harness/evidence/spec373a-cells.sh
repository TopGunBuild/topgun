...
usage() {
  cat >&2 <<'EOF'
usage: spec373a-cells.sh <cell>

  A COPY of spec372-allocdiag.sh, which is not edited. The difference list
  against that file is CLOSED, has exactly seven items, and is enumerated in
  the header block above.

  Every cell runs at csv cadence 60s, crash-interval 0, live-census ARMED at
  300s, log directive ARMED, journal default:
    d3e    flavour DH (dhat), server 61f84658, 300s, SIGTERM teardown
    d3l    flavour DH,        server 61f84658, 900s, SIGTERM teardown
    b1 b2  flavour CA (count-alloc), server 61f84658, 900s, SIGKILL teardown
    a1 a2  flavour CA, server = the freeze commit, 900s, SIGKILL teardown
  Every other matrix literal is spec372-allocdiag.sh's own.

  Any other argument falls through to this text and exits 2.

  The run REFUSES TO START, before any clock, if
  SPEC373A_CODE_FREEZE still reads its own placeholder value, or unless the
  ...
  SPEC373A_SERVER_COMMIT (the code state it was built from),
  SPEC373A_HARNESS_BIN and SPEC373A_CHAIN_START_EPOCH are REQUIRED, and are
  exported by spec373a-diag.sh / spec373a-chain.sh. This runner builds
  nothing.
EOF
  exit 2
}
...
#   flavour    -- CA|DH: the server build the cell must launch
...
# "pin" = 61f84658, "freeze" = SPEC373A_CODE_FREEZE.
case "$CELL" in
  d3e)   DURATION=300; FLAVOUR=DH; CELL_JOURNAL=default; CELL_GRACEFUL=yes; CELL_SERVER=pin ;;
  d3l)   DURATION=900; FLAVOUR=DH; CELL_JOURNAL=default; CELL_GRACEFUL=yes; CELL_SERVER=pin ;;
  b1|b2) DURATION=900; FLAVOUR=CA; CELL_JOURNAL=default; CELL_GRACEFUL=no;  CELL_SERVER=pin ;;
  a1|a2) DURATION=900; FLAVOUR=CA; CELL_JOURNAL=default; CELL_GRACEFUL=no;  CELL_SERVER=freeze ;;
  *)    usage ;;
esac
BASE="spec373a-${CELL}"
...
DATA_DIR="${SPEC365_DATA_DIR:-${REPO_ROOT}/target/spec373a-${CELL}-data}"
...
SERVER_PORT=47359       # fixed, distinct from spec362b-durable.sh's 47355,
                         # spec371-memdiag.sh's 47357 and spec372-allocdiag.sh's
                         # 47358 so no two runners' cells collide on one host
...
# ---------------------------------------------------------------------------
SPEC373A_CODE_FREEZE=4998d884
if [ "$SPEC373A_CODE_FREEZE" = "PENDING_SPEC373A_CODE_FREEZE" ]; then
  echo "FATAL: SPEC373A_CODE_FREEZE still reads its placeholder value." >&2
...
if ! FREEZE_RS_DIFF="$(git -C "$REPO_ROOT" diff --stat "$SPEC373A_CODE_FREEZE"..HEAD -- '*.rs' 2>&1)"; then
  echo "FATAL: the .rs diff against the freeze commit ${SPEC373A_CODE_FREEZE} could" >&2
...
if [ -n "$FREEZE_RS_DIFF" ]; then
  echo "FATAL: the .rs tree at HEAD differs from the freeze commit ${SPEC373A_CODE_FREEZE}; this run would not be filed under it" >&2
  printf '%s\n' "$FREEZE_RS_DIFF" >&2
  exit 1
fi
...
# This runner builds nothing: spec373a-diag.sh/spec373a-chain.sh build every flavour once, into
# its own target dir, and hands the paths over. Both are required.
if [ -z "${SPEC373A_HARNESS_BIN:-}" ] || [ ! -x "${SPEC373A_HARNESS_BIN}" ]; then
  echo "FATAL: SPEC373A_HARNESS_BIN must name the chain-built soak_harness binary." >&2
  exit 3
fi
case "${SPEC373A_CHAIN_START_EPOCH:-}" in
  ''|*[!0-9]*)
    echo "FATAL: SPEC373A_CHAIN_START_EPOCH must be the chain's integer start epoch." >&2
    exit 3
    ;;
esac
...
case "$CELL_SERVER" in
  pin)    EXPECT_SERVER_COMMIT=61f84658 ;;
  freeze) EXPECT_SERVER_COMMIT="$SPEC373A_CODE_FREEZE" ;;
esac
EXPECT_SERVER_SHA="$(git -C "$REPO_ROOT" rev-parse --verify "${EXPECT_SERVER_COMMIT}^{commit}" 2>/dev/null || true)"
GOT_SERVER_SHA="$(git -C "$REPO_ROOT" rev-parse --verify "${SPEC373A_SERVER_COMMIT:-none}^{commit}" 2>/dev/null || true)"
if [ -z "$EXPECT_SERVER_SHA" ] || [ "$GOT_SERVER_SHA" != "$EXPECT_SERVER_SHA" ]; then
  echo "FATAL: cell ${CELL} needs a server built at ${EXPECT_SERVER_COMMIT}; SPEC373A_SERVER_COMMIT='${SPEC373A_SERVER_COMMIT:-}'." >&2
  exit 3
fi
...
if [ "$SERVER_MTIME_EPOCH" -lt "$SPEC373A_CHAIN_START_EPOCH" ]; then
...
if [ "$(date -r "$SOAK_BIN" '+%s')" -lt "$SPEC373A_CHAIN_START_EPOCH" ]; then
...
{
  echo
  echo "=== spec373a carve 9c part a run: cell ${CELL} (flavour ${FLAVOUR}) ==="
  ...
  echo "  chain start:    ${SPEC373A_CHAIN_START_EPOCH}"
  ...
  echo "  code freeze:            ${SPEC373A_CODE_FREEZE}"
  ...
  echo "  server code:    ${CELL_SERVER} = ${EXPECT_SERVER_COMMIT} (SPEC373A_SERVER_COMMIT=${SPEC373A_SERVER_COMMIT}, resolves ${GOT_SERVER_SHA})"
...
# columns 42-45 of the parent's item 7, plus this file's item 5: the eight
# je_probe columns 46-53 and the alloc_probe byte counters 54-55; column 41,
# clean_mb, is the footprint
